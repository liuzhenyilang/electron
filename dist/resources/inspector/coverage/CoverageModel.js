...
reset(){this._coverageByURL=new Map();this._coverageByContentProvider=new Map();this.dispatchEventToListeners(CoverageModel.Events.CoverageReset);}
...
SDK.SDKModel.register(CoverageModel,SDK.Target.Capability.None,false);export class URLCoverageInfo extends Common.Object{constructor(url){super();this._url=url;this._coverageInfoByLocation=new Map();this._size=0;this._usedSize=0;this._type;this._isContentScript=false;}
...
_addToSizes(usedSize,size){this._usedSize+=usedSize;this._size+=size;if(usedSize!==0||size!==0){this.dispatchEventToListeners(Coverage.URLCoverageInfo.Events.SizesChanged);}}
