...
const legacyFileName=`${this._name}-legacy.js`;const fileName=this._descriptor.modules.includes(legacyFileName)?legacyFileName:`${this._name}.js`;return eval(`import('./${this._name}/${fileName}')`);}
...
class AppProvider{createApp(){}}
class CharacterIdMap{constructor(){this._elementToCharacter=new Map();this._characterToElement=new Map();this._charCode=33;}
...
return object;}}
function removeEventListeners(eventList){for(const eventInfo of eventList){eventInfo.eventTarget.removeEventListener(eventInfo.eventType,eventInfo.listener,eventInfo.thisObject);}
eventList.splice(0);}
...
EventTarget.removeEventListeners=removeEventListeners;class ObjectWrapper{constructor(){this._listeners;}
...
_updateVersionFrom3To4(){const advancedMode=Common.settings.createSetting('showHeaSnapshotObjectsHiddenProperties',false);moduleSetting('showAdvancedHeapSnapshotProperties').set(advancedMode.get());advancedMode.remove();}
...
class Color{constructor(rgba,format,originalText){this._hsla=undefined;this._rgba=rgba;this._originalText=originalText||null;this._originalTextIsValid=!!this._originalText;this._format=format;if(typeof this._rgba[3]==='undefined'){this._rgba[3]=1;}
...
static detectColorFormat(color){const cf=Format;let format;const formatSetting=moduleSetting('colorFormat').get();if(formatSetting===cf.Original){format=cf.Original;}else if(formatSetting===cf.RGB){format=(color.hasAlpha()?cf.RGBA:cf.RGB);}else if(formatSetting===cf.HSL){format=(color.hasAlpha()?cf.HSLA:cf.HSL);}else if(formatSetting===cf.HEX){format=color.detectHEXFormat();}else{format=cf.RGBA;}
return format;}
...
Color.hsva2rgba._tmpHSLA=[0,0,0,0];Color.calculateContrastRatio._blendedFg=[0,0,0,0];class Revealer{reveal(object,omitFocus){}}
...
return extension.descriptor()['destination'];};class Console extends ObjectWrapper{constructor(){super();this._messages=[];}
...
class ResourceType{constructor(name,title,category,isTextType){this._name=name;this._title=title;this._category=category;this._isTextType=isTextType;}
...
class Progress{setTotalWork(totalWork){}
setTitle(title){}
...
class QueryParamHandler{handleQueryParam(value){}}
class Runnable{run(){}}
class Segment{constructor(begin,end,data){if(begin>end){throw new Error('Invalid segment');}
...
class StaticContentProvider{constructor(contentURL,contentType,lazyContent){this._contentURL=contentURL;this._contentType=contentType;this._lazyContent=lazyContent;}
...
class TextDictionary{constructor(){this._words=new Map();this._index=new Trie();}
...
class Throttler{constructor(timeout){this._timeout=timeout;this._isRunningProcess=false;this._asSoonAsPossible=false;this._process=null;this._lastCompleteTime=0;this._schedulePromise=new Promise(fulfill=>{this._scheduleResolve=fulfill;});}
...
class WorkerWrapper{constructor(appName){let url=appName+'.js';url+=Root.Runtime.queryParamsString();this._workerPromise=new Promise(fulfill=>{this._worker=new Worker(url);this._worker.onmessage=onMessage.bind(this);function onMessage(event){console.assert(event.data==='workerReady');this._worker.onmessage=null;fulfill(this._worker);this._worker=null;}});}
...
self.Common=self.Common||{};Common=Common||{};Common.App=App;Common.AppProvider=AppProvider;Common.CharacterIdMap=CharacterIdMap;Common.Color=Color;Common.Color.Regex=Regex;Common.Color.Format=Format;Common.Color.Nicknames=Nicknames;Common.Color.PageHighlight=PageHighlight;Common.Color.Generator=Generator;Common.console=new Console();Common.Console=Console;Common.Console.Events=Events;Common.Console.MessageLevel=MessageLevel;Common.Console.Message=Message;Common.ContentProvider=ContentProvider;Common.ContentProvider.SearchMatch=SearchMatch;Common.ContentProvider.performSearchInContent=performSearchInContent;Common.ContentProvider.contentAsDataURL=contentAsDataURL;Common.EventTarget=EventTarget;Common.JavaScriptMetadata=JavaScriptMetaData;Common.Linkifier=Linkifier;Common.Object=ObjectWrapper;Common.ParsedURL=ParsedURL;Common.Progress=Progress;Common.CompositeProgress=CompositeProgress;Common.SubProgress=SubProgress;Common.ProgressProxy=ProgressProxy;Common.QueryParamHandler=QueryParamHandler;Common.resourceTypes=resourceTypes;Common.resourceCategories=resourceCategories;Common.ResourceCategory=ResourceCategory;Common.ResourceType=ResourceType;Common.Revealer=Revealer;Common.Revealer.reveal=reveal;Common.Revealer.revealDestination=revealDestination;Common.Runnable=Runnable;Common.Segment=Segment;Common.SegmentedRange=SegmentedRange;Common.Settings=Settings;Common.SettingsStorage=SettingsStorage;Common.Setting=Setting;Common.RegExpSetting=RegExpSetting;Common.settingForTest=settingForTest;Common.VersionController=VersionController;Common.moduleSetting=moduleSetting;Common.SettingStorageType=SettingStorageType;Common.StaticContentProvider=StaticContentProvider;Common.OutputStream=OutputStream;Common.StringOutputStream=StringOutputStream;Common.TextDictionary=TextDictionary;Common.Throttler=Throttler;Common.Trie=Trie;Common.UIStringFormat=UIStringFormat;Common.UIString=UIString;Common.serializeUIString=serializeUIString;Common.deserializeUIString=deserializeUIString;Common.localize=localize;Common.Worker=WorkerWrapper;Common.DeferredContent;Common.EventTarget.EventDescriptor;Common.Event;Common.Linkifier.Options;Common.Object._listenerCallbackTuple;Common.settings;Common.Throttler.FinishCallback;self.ls=ls$1;const HeapSnapshotProgressEvent={Update:'ProgressUpdate',BrokenSnapshot:'BrokenSnapshot'};const baseSystemDistance=100000000;class AllocationNodeCallers{constructor(nodesWithSingleCaller,branchingCallers){this.nodesWithSingleCaller=nodesWithSingleCaller;this.branchingCallers=branchingCallers;}}
