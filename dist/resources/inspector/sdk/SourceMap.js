...
static async load(sourceMapURL,compiledURL){let content=await new Promise((resolve,reject)=>{SDK.multitargetNetworkManager.loadResource(sourceMapURL,(statusCode,_headers,content,netError)=>{if(!content||statusCode>=400){const showInternalError=Root.Runtime.experiments.isEnabled('reportInternalNetErrorOnSourceMapLoadFail');const internalError=showInternalError?ls` (HTTP status code: ${statusCode}, net error code ${netError})`:``;const error=new Error(ls`Could not load content for ${sourceMapURL}${internalError}`);reject(error);}else{resolve(content);}});});if(content.slice(0,3)===')]}'){content=content.substring(content.indexOf('\n'));}
...
WasmSourceMap.FAKE_URL='wasm://dwarf';self.SDK=self.SDK||{};SDK=SDK||{};SDK.SourceMap=SourceMap;SDK.SourceMapEntry=SourceMapEntry;SDK.TextSourceMap=TextSourceMap;SDK.WasmSourceMap=WasmSourceMap;SDK.SourceMap.EditResult=EditResult;