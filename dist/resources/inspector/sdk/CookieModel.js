...
static cookieMatchesResourceURL(cookie,resourceURL){const url=Common.ParsedURL.fromString(resourceURL);if(!url||!CookieModel.cookieDomainMatchesResourceDomain(cookie.domain(),url.host)){return false;}
...
getCookiesForDomain(domain){const resourceURLs=[];function populateResourceURLs(resource){const documentURL=Common.ParsedURL.fromString(resource.documentURL);if(documentURL&&(!domain||documentURL.securityOrigin()===domain)){resourceURLs.push(resource.url);}}
