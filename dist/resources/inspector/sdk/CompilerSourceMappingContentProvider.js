...
requestContent(){return new Promise(resolve=>{SDK.multitargetNetworkManager.loadResource(this._sourceURL,(statusCode,_headers,content,netError)=>{if(statusCode>=400){const error=ls`Could not load content for ${this._sourceURL} (HTTP status code: ${
                  statusCode}, net error code ${netError})`;console.error(error);resolve({error,isEncoded:false});}else{resolve({content,isEncoded:false});}});});}
