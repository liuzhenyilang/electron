export default class HARWriter{static async write(stream,requests,progress){const compositeProgress=new Common.CompositeProgress(progress);const content=await Network.HARWriter._harStringForRequests(requests,compositeProgress);if(progress.isCanceled()){return Promise.resolve();}
...
progress.done();}}
export const _jsonIndent=2;export const _chunkSize=100000;self.Network=self.Network||{};Network=Network||{};Network.HARWriter=HARWriter;Network.HARWriter._jsonIndent=_jsonIndent;Network.HARWriter._chunkSize=_chunkSize;