export function _isDebugTest(){return!self.testRunner||!!Root.Runtime.queryParam('debugFrontend');}
export function _isStartupTest(){return Root.Runtime.queryParam('test').includes('/startup/');}
export function _consoleOutputHook(messageType){addResult(messageType+': '+Array.prototype.slice.call(arguments,1));}
export function _printDevToolsConsole(){if(_isDebugTest()){return;}
console.log=_consoleOutputHook.bind(null,'log');console.error=_consoleOutputHook.bind(null,'error');console.info=_consoleOutputHook.bind(null,'info');}
function completeTestOnError(message,source,lineno,colno,error){addResult('TEST ENDED IN ERROR: '+error.stack);completeTest();}
self['onerror']=completeTestOnError;_printDevToolsConsole();let _results=[];let _innerAddResult=text=>{_results.push(String(text));};export function addResult(text){_innerAddResult(text);}
let completed=false;let _innerCompleteTest=()=>{if(completed){return;}
completed=true;flushResults();self.testRunner.notifyDone();};export function completeTest(){_innerCompleteTest();}
self.TestRunner={_startupTestSetupFinished:()=>{}};export function setupStartupTest(path){const absoluteURL=url(path);const promise=new Promise(f=>TestRunner._startupTestSetupFinished=()=>{TestRunner._initializeTargetForStartupTest();delete TestRunner._startupTestSetupFinished;f();});self.testRunner.navigateSecondaryWindow(absoluteURL);return promise;}
...
_results=[];}
export function _executeTestScript(){const testScriptURL=(Root.Runtime.queryParam('test'));fetch(testScriptURL).then(data=>data.text()).then(testScript=>{if(_isDebugTest()){_innerAddResult=console.log;_innerCompleteTest=()=>console.log('Test completed');if(!self.testRunner){eval(`(function test(){${testScript}})()\n//# sourceURL=${testScriptURL}`);}else{self.eval(`function test(){${testScript}}\n//# sourceURL=${testScriptURL}`);}
return;}
testScript=testScript.trimRight();if(testScript.endsWith(';')){testScript=testScript.slice(0,testScript.length-1);}
(async function(){try{await eval(testScript+`\n//# sourceURL=${testScriptURL}`);}catch(err){addResult('TEST ENDED EARLY DUE TO UNCAUGHT ERROR:');addResult(err&&err.stack||err);addResult('=== DO NOT COMMIT THIS INTO -expected.txt ===');completeTest();}})();}).catch(error=>{addResult(`Unable to execute test script because of error: ${error}`);completeTest();});}
...
return promise;}
export function showPanel(panel){return UI.viewManager.showView(panel);}
...
const response=await TestRunner.RuntimeAgent.invoke_evaluate({expression:code,objectGroup:'console'});const error=response[Protocol.Error];if(error){addResult('Error: '+error);completeTest();return;}
return response;}
export async function evaluateInPageAnonymously(code,userGesture){const response=await TestRunner.RuntimeAgent.invoke_evaluate({expression:code,objectGroup:'console',userGesture});if(!response[Protocol.Error]){return response.result.value;}
addResult('Error: '+
...
export async function evaluateInPageAsync(code){const response=await TestRunner.RuntimeAgent.invoke_evaluate({expression:code,objectGroup:'console',includeCommandLineAPI:false,awaitPromise:true});const error=response[Protocol.Error];if(!error&&!response.exceptionDetails){return response.result.value;}
addResult('Error: '+
...
export function waitForTarget(filter){filter=filter||(target=>true);for(const target of SDK.targetManager.targets()){if(filter(target)){return Promise.resolve(target);}}
return new Promise(fulfill=>{const observer=({targetAdded:function(target){if(filter(target)){SDK.targetManager.unobserveTargets(observer);fulfill(target);}},targetRemoved:function(){},});SDK.targetManager.observeTargets(observer);});}
export function waitForTargetRemoved(targetToRemove){return new Promise(fulfill=>{const observer=({targetRemoved:function(target){if(target===targetToRemove){SDK.targetManager.unobserveTargets(observer);fulfill(target);}},targetAdded:function(){},});SDK.targetManager.observeTargets(observer);});}
...
export function hideInspectorView(){UI.inspectorView.element.setAttribute('style','display:none !important');}
...
export function waitForUISourceCodeRemoved(callback){Workspace.workspace.once(Workspace.Workspace.Events.UISourceCodeRemoved).then(callback);}
...
export async function dumpInspectedPageElementText(querySelector){const value=await evaluateInPageAsync(`document.querySelector('${querySelector}').innerText`);addResult(value);}
let _startedTest=false;export class _TestObserver{targetAdded(target){if(target.id()==='main'){_setupTestHelpers(target);}
if(_startedTest){return;}
_startedTest=true;if(_isStartupTest()){return;}
TestRunner.loadHTML(`
      <head>
        <base href="${url()}">
      </head>
